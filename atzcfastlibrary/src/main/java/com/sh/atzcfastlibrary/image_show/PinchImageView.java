...
public class PinchImageView extends ImageView {
     ...
     * @see #getPinchMode()
     */
    public static final int PINCH_MODE_DONW_CLOSE = 3;
    ...
    protected float getMaxScale() {
        float scale1 = (float) mScreenWH[0] / (float) getDrawable().getIntrinsicWidth();
        return MAX_SCALE + scale1;
    }
    ...
    private void initView() {
        ...
        mScreenWH = CommonUtils.getScreenHW(getContext());
    }
     ...
     * @see #fling(float, float)
     */
    private MovingImage mMovingImage = new MovingImage();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
            ...
            if (mPinchMode == PINCH_MODE_SCALE) {
                scaleEnd();
            } else if (mPinchMode == PINCH_MODE_DONW_CLOSE || mPinchMode == PINCH_MODE_SCROLL) {
                if (!isMoveScrolled) {
                    mMovingImage.checkDoFinishAct(event);
                }
            }
            mPinchMode = PINCH_MODE_FREE;
            isMoveScrolled = false;
            ...
            if (!(mScaleAnimator != null && mScaleAnimator.isRunning())) {
                ...
                if (mPinchMode == PINCH_MODE_SCROLL) {
                    //每次移动产生一个差值累积到图片位置上
                    boolean isVaild = scrollBy(event.getX() - mLastMovePoint.x, event.getY() - mLastMovePoint.y);
                    if (isVaild) {
                        isMoveScrolled = true;
                    }
                    if (!isMoveScrolled && !isVaild) {

                        float deltaX = event.getRawX() - mDownPoint.x;
                        float deltaY = event.getRawY() - mDownPoint.y;
                        if (deltaY > 0) {
                            mMovingImage.setupMovingAndZoom(event.getRawX(), event.getRawY(), mDownPoint.x, mDownPoint.y);
                        }
//                        addIntoVelocity(ev);
                    }
                    //记录新的移动点
    ...
    public interface CloseImageShowCb {
        public abstract void onPictureRelease();
    }

    private CloseImageShowCb mCloseImageShowCb;
    private View parentLayout;

    public void setCloseImageShowCb(CloseImageShowCb cb, View parentLayout) {
        mCloseImageShowCb = cb;
    ...
    private class MovingImage {
        public final float MIN_SCALE_WEIGHT = 0.25f;
        public final int REBACK_DURATION = 300;//ms

        private int screenHeight;
        protected VelocityTracker mVelocityTracker;

        public MovingImage() {
            screenHeight = CommonUtils.getScreenHW(getContext())[1];
        }

        // 检查决定是否关闭当前act
        public void checkDoFinishAct(MotionEvent ev) {
            final float mUpX = ev.getRawX();//->mDownX
            final float mUpY = ev.getRawY();//->mDownY

            float vY = computeYVelocity();
//            Log.e("pimg", "pimg, checkDoFinishAct() mUpX = " + mUpX + "; mUpY = " + mUpY);
//            Log.e("pimg", "pimg, checkDoFinishAct() mUpY - mDownPoint.y = " + (mUpY - mDownPoint.y));
//            Log.e("pimg", "pimg, checkDoFinishAct() screenHeight / 4 = " + (screenHeight / 4));
//            Log.e("pimg", "pimg, checkDoFinishAct() vY = " + vY);

            if (vY >= 1500 || Math.abs(mUpY - mDownPoint.y) > screenHeight / 4) {//速度有一定快，或者移动位置超过屏幕一半，那么释放
                if (mCloseImageShowCb != null) {
                    mCloseImageShowCb.onPictureRelease();
//                    Log.e("pimg", "pimg, checkDoFinishAct() Release");
                } else {
//                    Log.e("pimg", "pimg, checkDoFinishAct() Reback-1");
                    setupReback(mUpX, mUpY);
                }
            } else {
//                Log.e("pimg", "pimg, checkDoFinishAct() Reback-2");
                setupReback(mUpX, mUpY);
            }
        }
...
//            currentStatus = STATUS_REBACK;
            final float mDownX = mDownPoint.x;
            final float mDownY = mDownPoint.y;
            ...
            if (mUpY != mDownY) {
                ...
                valueAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationUpdate(ValueAnimator animation) {
                        ...
                        setupMovingAndZoom(mX, mY, mDownX, mDownY);
//                        if (mY == mDownY) {
//                            mDownY = 0;
//                            mDownX = 0;
...
//                        }
        ...
        private void setupMovingAndZoom(float movingX, float movingY, float downX, float downY) {
...
//            currentStatus = STATUS_MOVING;
            ...
            anim("translationX", deltaX);
            anim("translationY", deltaY);
            setupScale(scale);
        ...
        private void anim(String animType, float value) {
            ObjectAnimator objectAnimator = null;
            // form hide to show
            objectAnimator = ObjectAnimator.ofFloat(PinchImageView.this, animType, value);
        ...
        protected void setupBackground(float percent) {
            if (null != parentLayout) {
                parentLayout.setBackgroundColor(convertPercentToBlackAlphaColor(percent));
            }
        }
